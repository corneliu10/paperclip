# ClipHub: Marketplace for BirdAI Team Configurations

> The "app store" for whole-company AI teams — pre-built BirdAI configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for BirdAI-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | BirdAI company (agents, projects, governance) |
...
A complete BirdAI company configuration:
...
Single-agent configurations designed to plug into a BirdAI org:
...
Portable skill files that any BirdAI agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with BirdAI's skill loading system
...
interface Listing {
  ...
  birdaiVersionMin: string;     // Minimum BirdAI version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target BirdAI company
...
| `POST` | `/api/listings/:id/install` | Install to BirdAI company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target BirdAI company (or create new)
  → ClipHub API calls BirdAI API to:
  ...
  → Redirect to BirdAI dashboard with new team running
```
...
### 5.3 Creator: Export from BirdAI → Publish

```
Running BirdAI company → "Export as Blueprint" (CLI or UI)
  → BirdAI exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (BirdAI brand blue/purple)
...
| Install flow | One-click deploy to BirdAI company |
...
{
  "targetCompanyId": "uuid",         // Existing BirdAI company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `birdai-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as BirdAI
- **Backend**: Node.js API (or extend BirdAI server)
- **Database**: Postgres (can share BirdAI's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with BirdAI auth (or OAuth2)

### 9.2 Integration with BirdAI

ClipHub can be:
- **Option A**: A separate app that calls BirdAI's API to install blueprints
- **Option B**: A built-in section of the BirdAI UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing BirdAI UI and API.
...
- [ ] Install flow: blueprint → BirdAI company
...
- [ ] "Export from BirdAI" CLI command
