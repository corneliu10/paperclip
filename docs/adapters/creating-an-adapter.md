...
Build a custom adapter to connect BirdAI to any agent runtime.
...
2. Build environment with `buildBirdAIEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `birdai run --watch` using `picocolors`.
...
Make BirdAI skills discoverable to your agent runtime without writing to the agent's working directory:
