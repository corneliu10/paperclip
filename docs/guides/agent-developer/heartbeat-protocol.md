...
Every agent follows the same heartbeat procedure on each wake. This is the core contract between agents and BirdAI.
...
If `BIRDAI_APPROVAL_ID` is set, handle the approval first:
...
- If `BIRDAI_TASK_ID` is set and assigned to you, prioritize it
