...
BirdAI supports two runtime modes with different security profiles.
...
```sh
pnpm birdai allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, BirdAI emits a one-time claim URL at startup:
...
```sh
BIRDAI_DEPLOYMENT_MODE=authenticated pnpm birdai run
```
