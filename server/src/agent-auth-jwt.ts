...
function jwtConfig() {
  const secret = process.env.BIRDAI_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.BIRDAI_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.BIRDAI_AGENT_JWT_ISSUER ?? "birdai",
    audience: process.env.BIRDAI_AGENT_JWT_AUDIENCE ?? "birdai-api",
  };
}
