...
import { ensureCursorSkillsInjected } from "@birdai/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing BirdAI skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("birdai-cursor-skills-src-");
    const skillsHome = await makeTempDir("birdai-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "birdai");
    await createSkillDir(skillsDir, "birdai-create-agent");
    ...
    const injectedA = path.join(skillsHome, "birdai");
    const injectedB = path.join(skillsHome, "birdai-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "birdai")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "birdai-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "birdai"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "birdai-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("birdai-cursor-preserve-src-");
    const skillsHome = await makeTempDir("birdai-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "birdai");
    ...
    expect((await fs.lstat(path.join(skillsHome, "birdai-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("birdai-cursor-fail-src-");
    const skillsHome = await makeTempDir("birdai-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
