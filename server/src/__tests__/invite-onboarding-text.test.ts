...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("BirdAI OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested BirdAI base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("birdaiApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.birdaiApiUrl");
    expect(text).toContain("~/.openclaw/workspace/birdai-claimed-api-key.json");
    expect(text).toContain("BIRDAI_API_KEY");
