...
import { execute, testEnvironment } from "@birdai/adapter-openclaw-gateway/server";
import { parseOpenClawGatewayStdoutLine } from "@birdai/adapter-openclaw-gateway/ui";
import type { AdapterExecutionContext } from "@birdai/adapter-utils";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
      ...
      expect(payload?.sessionKey).toBe("birdai:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("BIRDAI_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("BIRDAI_TASK_ID=task-123");
