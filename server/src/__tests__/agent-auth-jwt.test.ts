...
describe("agent local JWT", () => {
  const secretEnv = "BIRDAI_AGENT_JWT_SECRET";
  const ttlEnv = "BIRDAI_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "BIRDAI_AGENT_JWT_ISSUER";
  const audienceEnv = "BIRDAI_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "birdai";
    process.env[audienceEnv] = "birdai-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
