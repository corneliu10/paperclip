...
const DEFERRED_WAKE_CONTEXT_KEY = "_birdaiWakeContext";
...
export function heartbeatService(db: Db) {
  ...
  async function executeRun(runId: string) {
    ...
    context.birdaiWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    try {
      ...
      for (const warning of runtimeWorkspaceWarnings) {
        await onLog("stderr", `[birdai] ${warning}\n`);
      }
      ...
      if (adapter.supportsLocalAgentJwt && !authToken) {
        logger.warn(
          ...
          "local agent jwt secret missing or invalid; running without injected BIRDAI_API_KEY",
        );
      }
