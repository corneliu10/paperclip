...
import { normalizeAgentUrlKey, portabilityManifestSchema } from "@birdai/shared";
...
async function readAgentInstructions(agent: AgentLike): Promise<{ body: string; warning: string | null }> {
  ...
  if (instructionsFilePath) {
    const workspaceCwd = asString(process.env.BIRDAI_WORKSPACE_CWD);
...
export function companyPortabilityService(db: Db) {
  ...
  async function resolveSource(source: CompanyPortabilityPreview["source"]): Promise<ResolvedSource> {
    ...
    let ref = parsed.ref;
    const manifestRelativePath = [parsed.basePath, "birdai.manifest.json"].filter(Boolean).join("/");
