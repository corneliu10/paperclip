...
describe("BirdAIApiClient", () => {
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new BirdAIApiClient({
    ...
    expect(headers["x-birdai-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new BirdAIApiClient({ apiBase: "http://localhost:3100" });
