...
const DATA_DIR_OPTION_HELP =
  "BirdAI data directory root (isolates state from ~/.birdai)";

program
  .name("birdai")
  .description("BirdAI CLI — setup, diagnose, and configure your instance")
  .version("0.2.7");
...
program
  ...
  .option("--run", "Start BirdAI immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your BirdAI setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "birdai")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run BirdAI")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the BirdAI server API")
