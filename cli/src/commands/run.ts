...
export async function runCommand(opts: RunOptions): Promise<void> {
  const instanceId = resolveBirdAIInstanceId(opts.instance);
  process.env.PAPERCLIP_INSTANCE_ID = instanceId;

  const homeDir = resolveBirdAIHomeDir();
  ...
  p.intro(pc.bgCyan(pc.black(" birdai run ")));
  ...
  if (!configExists(configPath)) {
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
      ...
      p.log.message(`Run ${pc.cyan("birdai onboard")} once, then retry ${pc.cyan("birdai run")}.`);
      process.exit(1);
    }
  ...
  p.log.step("Starting BirdAI server...");
...
function maybeEnableUiDevMiddleware(entrypoint: string): void {
  ...
  if (normalized.endsWith("/server/src/index.ts") || normalized.endsWith("@birdai/server/src/index.ts")) {
...
async function importServerEntry(): Promise<StartedServer> {
  ...
  // Production mode: import the published @birdai/server package
  try {
    const mod = await import("@birdai/server");
    return await startServerFromModule(mod, "@birdai/server");
  } catch (err) {
    const missingSpecifier = getMissingModuleSpecifier(err);
    const missingServerEntrypoint = !missingSpecifier || missingSpecifier === "@birdai/server";
    if (isModuleNotFoundError(err) && missingServerEntrypoint) {
      throw new Error(
        `Could not locate a BirdAI server entrypoint.\n` +
          `Tried: ${devEntry}, @birdai/server\n` +
          `${formatError(err)}`,
      );
    }
    throw new Error(
      `BirdAI server failed to start.\n` +
        `${formatError(err)}`,
    );
  }
}

function shouldGenerateBootstrapInviteAfterStart(config: BirdAIConfig): boolean {
...
async function startServerFromModule(mod: unknown, label: string): Promise<StartedServer> {
  ...
  if (typeof startServer !== "function") {
    throw new Error(`BirdAI server entrypoint did not export startServer(): ${label}`);
  }
