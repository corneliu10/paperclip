...
import { ApiRequestError, BirdAIApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to BirdAI config file")
    .option("-d, --data-dir <path>", "BirdAI data directory root (isolates state from ~/.birdai)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the BirdAI API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.BIRDAI_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const apiKey =
    options.apiKey?.trim() ||
    process.env.BIRDAI_API_KEY?.trim() ||
    readKeyFromProfileEnv(profile);

  const companyId =
    options.companyId?.trim() ||
    process.env.BIRDAI_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set BIRDAI_COMPANY_ID, or set context profile companyId via `birdai context set`.",
    );
  }

  const api = new BirdAIApiClient({ apiBase, apiKey });
  return {
    api,
    companyId,
    profileName,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.BIRDAI_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.BIRDAI_SERVER_PORT || "");
