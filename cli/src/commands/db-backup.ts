...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@birdai/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://birdai:birdai@127.0.0.1:${port}/birdai`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" birdai db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveBirdAIInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "birdai";
