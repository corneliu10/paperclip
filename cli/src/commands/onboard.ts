...
type OnboardDefaults = Pick<BirdAIConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const ONBOARD_ENV_KEYS = [
  "BIRDAI_PUBLIC_URL",
  "DATABASE_URL",
  "BIRDAI_DB_BACKUP_ENABLED",
  "BIRDAI_DB_BACKUP_INTERVAL_MINUTES",
  "BIRDAI_DB_BACKUP_RETENTION_DAYS",
  "BIRDAI_DB_BACKUP_DIR",
  "BIRDAI_DEPLOYMENT_MODE",
  "BIRDAI_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  ...
  "BIRDAI_STORAGE_LOCAL_DIR",
  "BIRDAI_STORAGE_S3_BUCKET",
  "BIRDAI_STORAGE_S3_REGION",
  "BIRDAI_STORAGE_S3_ENDPOINT",
  "BIRDAI_STORAGE_S3_PREFIX",
  "BIRDAI_STORAGE_S3_FORCE_PATH_STYLE",
  "BIRDAI_SECRETS_PROVIDER",
  "BIRDAI_SECRETS_STRICT_MODE",
  "BIRDAI_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(): {
  ...
  const instanceId = resolveBirdAIInstanceId();
  const defaultStorage = defaultStorageConfig();
  const defaultSecrets = defaultSecretsConfig();
  const databaseUrl = process.env.DATABASE_URL?.trim() || undefined;
  const publicUrl =
    process.env.BIRDAI_PUBLIC_URL?.trim() ||
    process.env.BIRDAI_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.BIRDAI_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.BIRDAI_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.BIRDAI_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.BIRDAI_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.BIRDAI_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.BIRDAI_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.BIRDAI_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.BIRDAI_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.BIRDAI_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.BIRDAI_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.BIRDAI_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.BIRDAI_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.BIRDAI_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.BIRDAI_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.BIRDAI_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.BIRDAI_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.BIRDAI_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.BIRDAI_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "BIRDAI_DEPLOYMENT_EXPOSURE",
...
function canCreateBootstrapInviteImmediately(config: Pick<BirdAIConfig, "database" | "server">): boolean {
  return config.server.deploymentMode === "authenticated" && config.database.mode !== "embedded-postgres";
}

export async function onboard(opts: OnboardOptions): Promise<void> {
  printBirdAICliBanner();
  p.intro(pc.bgCyan(pc.black(" birdai onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveBirdAIInstanceId());
  p.log.message(
    pc.dim(
  ...
  let llm: BirdAIConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@birdai/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `birdai doctor`"));
      }
    }
  ...
  if (jwtSecret.created) {
    p.log.success(`Created ${pc.cyan("BIRDAI_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  } else if (process.env.BIRDAI_AGENT_JWT_SECRET?.trim()) {
    p.log.info(`Using existing ${pc.cyan("BIRDAI_AGENT_JWT_SECRET")} from environment`);
  } else {
    p.log.info(`Using existing ${pc.cyan("BIRDAI_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  }

  const config: BirdAIConfig = {
    $meta: {
      version: 1,
  ...
  p.note(
    [
      ...
      "Agent auth: BIRDAI_AGENT_JWT_SECRET configured",
    ].join("\n"),
    "Configuration saved",
  );
  ...
  p.note(
    [
      `Run: ${pc.cyan("birdai run")}`,
      `Reconfigure later: ${pc.cyan("birdai configure")}`,
      `Diagnose setup: ${pc.cyan("birdai doctor")}`,
    ].join("\n"),
    "Next commands",
  );
  ...
  if (shouldRunNow && !opts.invokedByRun) {
    process.env.BIRDAI_OPEN_ON_LISTEN = "true";
