...
function defaultConfig(): BirdAIConfig {
...
export async function configure(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(" birdai configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `birdai onboard` first.");
    p.outro("");
    return;
  }
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because BIRDAI_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
