/**
 * esbuild configuration for building the birdai CLI for npm.
 *
 * Bundles all workspace packages (@birdai/*) into a single file.
...
// to npm and resolved at runtime (e.g. @birdai/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@birdai/")) {
      externals.add(name);
    }
  }
