...
type BirdAISprite = (typeof BIRDAI_SPRITES)[number];
...
function spriteSize(sprite: BirdAISprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = BIRDAI_SPRITES[(baseCol + baseRow) % BIRDAI_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = BIRDAI_SPRITES[Math.floor(Math.random() * BIRDAI_SPRITES.length)]!;
