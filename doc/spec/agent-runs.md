...
This spec defines how BirdAI actually runs agents while staying runtime-agnostic.
...
1. BirdAI is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, BirdAI manages lifecycle, and on exit BirdAI parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `birdai.skill` (shared BirdAI skill text block)
...
2. Preferred transport is env vars (`BIRDAI_*`) injected at runtime.
