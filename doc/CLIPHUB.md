...
ClipHub is the public registry where people share, discover, and download BirdAI company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to BirdAI what a package registry is to a programming language. BirdAI already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in BirdAI — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own BirdAI instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the BirdAI spec. It contains:
...
**Install (fresh start):**
```
birdai install birdhub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local BirdAI instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within BirdAI, export your company as a template, then publish:

```
birdai export --template my-company
birdai publish birdhub my-company
```
...
ClipHub is a **separate service** from BirdAI itself. BirdAI is self-hosted; ClipHub is a hosted registry that BirdAI instances talk to.
...
| **BirdAI CLI** | `birdai install`, `birdai publish`, `birdai birdhub sync` — built into BirdAI |
| **BirdAI UI** | "Browse ClipHub" panel in the BirdAI web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with BirdAI) |
| Backend | TypeScript + Hono (consistent with BirdAI) |
| Database | PostgreSQL |
...
4. Run `birdai install birdhub:acme/lean-saas-shop`
5. BirdAI creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in BirdAI until it works well
2. Export: `birdai export --template my-agency`
3. Publish: `birdai publish birdhub my-agency`
...
3. Install just that agent: `birdai install birdhub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to BirdAI

ClipHub is **not required** to use BirdAI. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to BirdAI what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `birdai install birdhub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in BirdAI web UI
- [ ] `birdai birdhub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with BirdAI's own philosophy)
