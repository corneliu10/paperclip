# BirdAI Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for BirdAI's plugin and extension architecture.
...
BirdAI plugin design is based on the following assumptions:

1. BirdAI is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core BirdAI business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by BirdAI core.
...
2. Let plugins add major capabilities without editing BirdAI core.
...
4. Depend on project-local plugin folders such as `.birdai/plugins`.
...
The single BirdAI deployment an operator installs and controls.

### 5.2 Company

A first-class BirdAI business object inside the instance.
...
A trusted in-process extension loaded directly by BirdAI core.
...
An installable instance-wide extension package loaded through the BirdAI plugin runtime.
...
BirdAI has two extension classes.
...
BirdAI already has a concrete workspace model:
...
Plugins live under the BirdAI instance directory.

Suggested layout:

- `~/.birdai/instances/default/plugins/package.json`
- `~/.birdai/instances/default/plugins/node_modules/`
- `~/.birdai/instances/default/plugins/.cache/`
- `~/.birdai/instances/default/data/plugins/<plugin-id>/`

The package install directory and the plugin data directory are separate.

## 8.2 Operator Commands

BirdAI should add CLI commands:

- `pnpm birdai plugin list`
- `pnpm birdai plugin install <package[@version]>`
- `pnpm birdai plugin uninstall <plugin-id>`
- `pnpm birdai plugin upgrade <plugin-id> [version]`
- `pnpm birdai plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@birdai/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface BirdAIPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that BirdAI agents can use during runs.
...
- BirdAI server starts one worker process per installed plugin
...
Receives one typed BirdAI domain event.
...
export function definePlugin(definition: PluginDefinition): BirdAIPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: BirdAIPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@birdai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@birdai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@birdai/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@birdai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@birdai/plugin-sdk/ui`
...
The `@birdai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the BirdAI secret provider system rather than a plain text input
...
1. Core BirdAI data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing BirdAI objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual BirdAI product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if BirdAI later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing BirdAI secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm birdai plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the BirdAI server. This is a normative requirement, not optional.
...
### 27.1 `@birdai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@birdai/plugin-test-harness";
...
For developing a plugin against a running BirdAI instance:

- The operator installs the plugin from a local path: `pnpm birdai plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-birdai-plugin`) that scaffolds:

- `package.json` with correct `birdaiPlugin` keys
...
- `@birdai/plugin-workspace-files`
- `@birdai/plugin-terminal`
- `@birdai/plugin-git`
- `@birdai/plugin-linear`
- `@birdai/plugin-github-issues`
- `@birdai/plugin-grafana`
- `@birdai/plugin-runtime-processes`
- `@birdai/plugin-stripe`
...
- `@birdai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@birdai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@birdai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@birdai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@birdai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@birdai/plugin-sdk/ui`
...
- `@birdai/plugin-test-harness`
- `create-birdai-plugin` starter template
...
BirdAI should not implement a generic in-process hook bag modeled directly after local coding tools.

BirdAI should implement:
...
That is the complete target design for the BirdAI plugin system.
