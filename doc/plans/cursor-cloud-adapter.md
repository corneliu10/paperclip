...
This document defines the V1 design for a BirdAI adapter that integrates with
...
4. synthesized stdout events for BirdAI UI/CLI
...
3. **Agent callback auth to BirdAI** uses a bootstrap exchange flow (no long-lived BirdAI key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from BirdAI endpoints, not full SKILL.md prompt injection.
...
- `birdaiPublicUrl` (optional override; else `BIRDAI_PUBLIC_URL` env)
...
## BirdAI Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `BIRDAI_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.birdaiPublicUrl`
2. `process.env.BIRDAI_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived BirdAI credentials in prompt text.

Flow:

1. Before launch/follow-up, BirdAI mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. BirdAI validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all BirdAI API calls.
...
1. Prompt includes a compact instruction to fetch skills from BirdAI.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/birdai`
   - `GET /api/skills/birdai-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <birdaiPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active BirdAI run context.
...
5. `birdaiPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from BirdAI skill endpoints |
| BirdAI auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from BirdAI context.
3. Add richer PR metadata surfacing in BirdAI UI.
