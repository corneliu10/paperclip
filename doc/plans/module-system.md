# BirdAI Module System

## Overview

BirdAI's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends BirdAI's API, UI, and data model | Yes |
...
modules/
  observability/
    birdai.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@birdai/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@birdai/core";
...
```
1. Scan modules/ directory for birdai.module.json manifests
...
```jsonc
// birdai.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@birdai/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@birdai/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:birdai/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.birdai.com/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm birdai store list                    # browse available modules and templates
pnpm birdai store install <module-id>     # install a module
pnpm birdai store import <template-id>    # import a company template
pnpm birdai store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one BirdAI instance | `server:started` |
...
4. **Module config** — `birdai.config.json` with per-module config, env var interpolation
...
10. **`@birdai/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
