...
- New config/env settings:
  - `BIRDAI_AGENT_JWT_SECRET`
  - `BIRDAI_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `BIRDAI_AGENT_JWT_ISSUER` (default: `birdai`)
  - `BIRDAI_AGENT_JWT_AUDIENCE` (default: `birdai-api`)
...
1. In:
   ...
   inject `BIRDAI_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `BIRDAI_API_KEY`, do not overwrite it.
- Continue injecting:
  - `BIRDAI_AGENT_ID`
  - `BIRDAI_COMPANY_ID`
  - `BIRDAI_API_URL`
