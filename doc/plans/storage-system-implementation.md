...
Add a single storage subsystem for BirdAI that supports:
...
- API serves attachment bytes through authenticated BirdAI endpoints.
...
- Default local path is under instance root: `~/.birdai/instances/<instanceId>/data/storage`.
...
- `birdai onboard` writes a valid `storage` config block by default.
- `birdai configure --section storage` can switch between local and s3 modes.
...
- `birdai doctor` reports actionable storage status.
