...
Agents need API keys to authenticate with BirdAI. The current approach
...
**Trust model:** The adapter process runs on the same machine as the BirdAI
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** BirdAI generates a token and passes it directly to the agent
...
`BIRDAI_API_URL`, `BIRDAI_AGENT_ID`, `BIRDAI_COMPANY_ID`. We need to
add a `BIRDAI_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a BirdAI CLI
...
```
birdai auth login
# Opens browser -> user confirms -> token stored at ~/.birdai/credentials
```
...
managed by the BirdAI server itself.
...
1. A company admin (user or agent) generates an **invite URL** from BirdAI.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The BirdAI SKILL.md (or a link to it)
   - What information BirdAI needs from the agent (e.g. webhook URL, adapter
...
5. BirdAI stores the pending registration.
...
7. On approval, BirdAI provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** BirdAI issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.birdai.com/skills/birdai/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How BirdAI should send you heartbeats",
...
1. Generate a BirdAI invite link for the company.
...
4. A BirdAI company member approves the new agent.
5. BirdAI begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `BIRDAI_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `birdai auth login` for developer-managed remote agents.                                      |
