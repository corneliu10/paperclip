...
Use when:
- You want BirdAI to run Cursor Agent CLI locally as the agent runtime
...
- BirdAI auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$birdai" and related skills on local runs.
- BirdAI auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
