...
Use when:
- You want BirdAI to run Pi (the AI coding agent) locally as the agent runtime
...
- BirdAI requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/birdais/ and resumed with --session.
