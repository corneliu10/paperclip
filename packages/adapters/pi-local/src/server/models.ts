...
import type { AdapterModel } from "@birdai/adapter-utils";
import { asString, runChildProcess } from "@birdai/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.BIRDAI_PI_COMMAND === "string" &&
    process.env.BIRDAI_PI_COMMAND.trim().length > 0
      ? process.env.BIRDAI_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["BIRDAI_", "npm_", "NPM_"] as const;
