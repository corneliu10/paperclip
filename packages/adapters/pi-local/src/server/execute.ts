...
const BIRDAI_SESSIONS_DIR = path.join(os.homedir(), ".pi", "birdais");
...
async function ensurePiSkillsInjected(onLog: AdapterExecutionContext["onLog"]) {
  const skillsDir = await resolveBirdAISkillsDir();
  if (!skillsDir) return;
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[birdai] Injected Pi skill "${entry.name}" into ${piSkillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[birdai] Failed to inject Pi skill "${entry.name}" into ${piSkillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function ensureSessionsDir(): Promise<string> {
  await fs.mkdir(BIRDAI_SESSIONS_DIR, { recursive: true });
  return BIRDAI_SESSIONS_DIR;
}

function buildSessionPath(agentId: string, timestamp: string): string {
  const safeTimestamp = timestamp.replace(/[:.]/g, "-");
  return path.join(BIRDAI_SESSIONS_DIR, `${safeTimestamp}-${agentId}.jsonl`);
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (wakeTaskId) env.BIRDAI_TASK_ID = wakeTaskId;
  if (wakeReason) env.BIRDAI_WAKE_REASON = wakeReason;
  if (wakeCommentId) env.BIRDAI_WAKE_COMMENT_ID = wakeCommentId;
  if (approvalId) env.BIRDAI_APPROVAL_ID = approvalId;
  if (approvalStatus) env.BIRDAI_APPROVAL_STATUS = approvalStatus;
  if (linkedIssueIds.length > 0) env.BIRDAI_LINKED_ISSUE_IDS = linkedIssueIds.join(",");
  if (workspaceCwd) env.BIRDAI_WORKSPACE_CWD = workspaceCwd;
  if (workspaceSource) env.BIRDAI_WORKSPACE_SOURCE = workspaceSource;
  if (workspaceId) env.BIRDAI_WORKSPACE_ID = workspaceId;
  if (workspaceRepoUrl) env.BIRDAI_WORKSPACE_REPO_URL = workspaceRepoUrl;
  if (workspaceRepoRef) env.BIRDAI_WORKSPACE_REPO_REF = workspaceRepoRef;
  if (workspaceHints.length > 0) env.BIRDAI_WORKSPACES_JSON = JSON.stringify(workspaceHints);
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stderr",
      `[birdai] Pi session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (resolvedInstructionsFilePath) {
    try {
      ...
      await onLog(
        "stderr",
        `[birdai] Loaded agent instructions file: ${resolvedInstructionsFilePath}\n`,
      );
    } catch (err) {
      ...
      await onLog(
        "stderr",
        `[birdai] Warning: could not read agent instructions file "${resolvedInstructionsFilePath}": ${reason}\n`,
      );
  ...
  if (
    ...
    await onLog(
      "stderr",
      `[birdai] Pi session "${runtimeSessionId}" is unavailable; retrying with a fresh session.\n`,
    );
