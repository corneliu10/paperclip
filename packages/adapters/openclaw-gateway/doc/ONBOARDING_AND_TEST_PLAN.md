...
## Scope
This plan is now **gateway-only**. BirdAI supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses BirdAI skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a BirdAI task.
...
```bash
pnpm --filter @birdai/server test:run -- openclaw-gateway
pnpm --filter @birdai/server typecheck
pnpm --filter @birdai/ui typecheck
pnpm --filter birdai typecheck
```
