...
} from "@birdai/adapter-utils";
import { asString, parseObject } from "@birdai/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the BirdAI server host.",
        });
      }
    } catch (err) {
      checks.push({
