...
COMPANY_ID="${COMPANY_ID:-${BIRDAI_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-birdai-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-birdai-openclaw-smoke}"
...
Provide one of:
  BIRDAI_AUTH_HEADER=\"Bearer <board-token>\"
  BIRDAI_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "BirdAI OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.birdai.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
