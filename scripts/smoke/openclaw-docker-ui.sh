...
BIRDAI_HOST_PORT="${BIRDAI_HOST_PORT:-3100}"
BIRDAI_HOST_FROM_CONTAINER="${BIRDAI_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.birdai-openclaw.override.yml"
...
detect_birdai_base_url() {
  ...
  for candidate in "$BIRDAI_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${BIRDAI_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${BIRDAI_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
birdai_base_url="$(detect_birdai_base_url || true)"
...
BirdAI URL for OpenClaw container:
EOF
  if [[ -n "$birdai_base_url" ]]; then
    cat <<EOF
  $birdai_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${BIRDAI_HOST_PORT}
  (Do not use http://127.0.0.1:${BIRDAI_HOST_PORT} inside the container.)
  If BirdAI rejects the host, run on host machine:
    pnpm birdai allowed-hostname host.docker.internal
  Then restart BirdAI and re-run this script.
EOF
  fi
  cat <<EOF
